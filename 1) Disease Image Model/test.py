...
try:
    ...
    exit()
print("\nTesting predictions...")
val_img_dir = 'augmented_disease_dataset/images/val'
if os.path.exists(val_img_dir):
    ...
    if val_images:
        ...
        try:
            ...
            for i, result in enumerate(results):
                ...
                if hasattr(result, 'masks') and result.masks is not None:
                    ...
                    print(f"Detected {num_detections} disease spots")
                    if hasattr(result, 'boxes') and result.boxes is not None:
                        confidences = result.boxes.conf.cpu().numpy()
                        print(f"Confidence scores: {[f'{conf:.2f}' for conf in confidences]}")
                else:
                    print("No disease spots detected")
            print(f"Results saved to: runs/segment/predict/")
        except Exception as e:
        ...
        try:
            ...
            print(f"All results saved to: runs/segment/predict/")
            total_detections = 0
            for i, result in enumerate(batch_results):
                if hasattr(result, 'masks') and result.masks is not None:
                    num_detections = len(result.masks)
                    total_detections += num_detections
                    print(f"Image {i+1}: {num_detections} disease spots detected")
                else:
                    print(f"Image {i+1}: No disease spots detected")
